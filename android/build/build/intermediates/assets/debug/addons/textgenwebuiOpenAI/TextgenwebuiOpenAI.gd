...
@export var num_cache_messages = 6 # (int, 0, 20)
...
func _ready():
	# Change port if running on quest native (requires --listen command line argument in textgenwebui)
	if OS.has_feature("android"):
		url = "http://0.0.0.1:5001/v1/chat/completions"
		streaming_url = "http://0.0.0.1"
...
func _on_text_received(new_text : String):
	if !new_text.begins_with("*") and !new_text.ends_with("*"):
		...
		if new_text == "." or new_text == "?" or new_text == "!" or new_text == "," or new_text == ";":
			...
			# Store most recent response if using messages cache
			if num_cache_messages > 0:
				past_messages_array.append({"role": "assistant", "content": ai_response})
